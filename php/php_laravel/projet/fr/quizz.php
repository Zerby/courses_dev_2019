    ...
    echo '<div class="card-panel grey lighten-5 z-depth-1">
                <div class="row valign-wrapper">
                    <div class="col s12 center-align margin">
                        <h2>Bonne réponse !</h2>
												<p>Pour accéder à la suite des cours, c\'est <a href="dashboard.php">ici</a></p>
                    </div>
                </div>
            </div>'; 
    $quizz = "UPDATE `reussi` SET reussi1 = 1 WHERE id_quizz = :id_quizz AND id = :id";
    $stmt2 = $pdo->prepare($quizz);
    $stmt2->bindValue(':id_quizz', $cours1['id_cours'], PDO::PARAM_INT);
		$stmt2->bindValue(':id', $_SESSION['id'], PDO::PARAM_INT);
    $stmt2->execute();
}else{
       echo '<div class="card-panel grey lighten-5 z-depth-1">
                <div class="row valign-wrapper">
                    <div class="col s12 center-align margin">
                        <h2>Essayez encore et n\'hésitez pas à regarder le cours.</h2>
                    </div>
                </div>
            </div>'; 
}
